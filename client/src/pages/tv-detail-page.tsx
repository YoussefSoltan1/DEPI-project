        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1">
            <h1 className="text-4xl font-bold text-white mb-2">
              {tvShow.name} <span className="text-textSecondary">({getYear(tvShow.first_air_date)})</span>
            </h1>
            ...
            {tvShow.created_by && tvShow.created_by.length > 0 && (
              <div className="mb-8">
                <h3 className="text-xl font-semibold text-white mb-3">Created By</h3>
                <div className="flex flex-wrap gap-4">
                  {tvShow.created_by.map((creator) => (
                    <div key={creator.id} className="flex items-center">
                      <Avatar className="h-8 w-8 mr-2">
                        {creator.profile_path ? (
                          <AvatarImage src={getImageUrl(creator.profile_path, 'w92')} />
                        ) : null}
                        <AvatarFallback>
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              ...
              <Button variant="outline" className="border-hoverBg text-white hover:bg-hoverBg">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
          ...
          <Tabs defaultValue="cast">
            ...
            <TabsContent value="cast" className="mt-6">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {tvShow.credits?.cast?.slice(0, 12).map((person) => (
                  <Card key={person.id} className="bg-hoverBg border-none overflow-hidden">
                    <CardContent className="p-0">
                      <div className="relative pb-4">
                        <Avatar className="h-40 w-full rounded-none">
                          {person.profile_path ? (
                            <AvatarImage src={getImageUrl(person.profile_path, 'w185')} />
                          ) : null}
                          <AvatarFallback className="rounded-none h-40 text-lg">
                            <User className="h-10 w-10" />
                          </AvatarFallback>
                        </Avatar>
                        <div className="p-3">
                          <h4 className="font-medium text-white truncate">{person.name}</h4>
                          <p className="text-xs text-textSecondary truncate">{person.character}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card key={person.id} className="bg-hoverBg border-none overflow-hidden">
                    <CardContent className="p-0">
                      <div className="relative pb-4">
                        ...
                        <div className="p-3">
                          <h4 className="font-medium text-white truncate">{person.name}</h4>
                          <p className="text-xs text-textSecondary truncate">{person.job}</p>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
            ...
            <TabsContent value="details" className="mt-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <h3 className="text-xl font-semibold text-white mb-4">TV Show Information</h3>
                  <div className="space-y-3">
                    ...
                    <div>
                      <span className="text-textSecondary">First Air Date: </span>
                      <span className="text-white">{new Date(tvShow.first_air_date).toLocaleDateString()}</span>
                    </div>
                    <div>
                      <span className="text-textSecondary">Number of Seasons: </span>
                      <span className="text-white">{tvShow.number_of_seasons}</span>
                    </div>
                    <div>
                      <span className="text-textSecondary">Number of Episodes: </span>
                      <span className="text-white">{tvShow.number_of_episodes}</span>
                    </div>
                  </div>
                </div>
                <div>
