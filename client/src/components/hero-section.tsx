    ...
    <section className="relative h-[500px] md:h-[600px] overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-20 h-full flex flex-col justify-center">
        <div className="max-w-2xl">
          <span className="inline-block px-3 py-1 bg-secondary text-white rounded-full text-sm font-medium mb-4">
            New Release
          </span>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {genreNames.map((name, index) => (
              <span key={index} className="bg-hoverBg px-3 py-1 rounded-full text-sm text-white">
                {name}
              </span>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-4">
            <Button className="bg-secondary hover:bg-red-700 transition-colors duration-200 text-white py-3 px-6 rounded-full flex items-center">
              <Play className="h-4 w-4 mr-2" /> Watch Now
            </Button>
            <Link href={`/movie/${movie.id}`}>
              <Button className="bg-hoverBg hover:bg-gray-700 transition-colors duration-200 text-white py-3 px-6 rounded-full flex items-center">
